...
// File created 15 Feb 2012 10:56:55 for Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.1; Trident/5.0; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; .NET4.0C; .NET4.0E) IP 14.145.226.134
// crc32=1607029405
...
var mxLog = {
	...
	init: function() {
		if (mxLog.window == null && document.body != null) {
			...
			if ((false || true) && !false && !false && document.compatMode != 'BackCompat') {
...
var mxUtils = {
	...
	findNodeByAttribute: function() {
		if (document.documentMode == 9) {
			...
			return function(node, attr, value) {
				var result = node.ownerDocument.evaluate('//*[@' + attr + '=\'' + value + '\']', node.ownerDocument, null, XPathResult.ANY_TYPE, null);
	...
	parseXml: function() {
		if (true && document.documentMode != 9) {
			return function(xml) {
	...
	arcToCurves: function(x0, y0, r1, r2, angle, largeArcFlag, sweepFlag, x, y) {
		...
		for (var n = 0; n < seg; ++n) {
			...
			result = result.concat([Number(x2 + x0), Number(y2 + y0), Number(x3 - dx + x0), Number(y3 - dy + y0), Number(x3 + x0), Number(y3 + y0)]);
			x2 = x3 + dx;
			y2 = y3 + dy;
		}
		return result;
	...
	setOpacity: function(node, value) {
		...
		} else if (true && document.documentMode != 9) {
			if (value >= 100) {
				node.style.filter = null;
			} else {
...
mxImageExport.prototype.drawShape = function(state, canvas, shape) {
	...
	if (shape.isExportWrapperShape && (direction == 'south' || direction == 'north')) {
	...
	var imageShape = state.style[mxConstants.STYLE_SHAPE] == 'image';
	...
	var glass = (fillColor != null && (state.style[mxConstants.STYLE_SHAPE] == 'label' || state.style[mxConstants.STYLE_SHAPE] == 'rectangle'));
	...
	if (imageShape || state.style[mxConstants.STYLE_SHAPE] == 'label') {
...
mxImageExport.prototype.drawLabel = function(state, canvas, bounds, vert, str) {
	...
	var y = bounds.y - mxUtils.getValue(state.style, mxConstants.STYLE_LABEL_PADDING, 0);
	canvas.text(bounds.x, y, bounds.width, bounds.height, str, align, null, vert);
};
...
mxImageExport.prototype.drawMarker = function(canvas, state, source) {
	...
	var strokeX = unitX * sw;
	var strokeY = unitY * sw;
	pe = pe.clone();
	...
	if (f != null) {
		offset = f(canvas, state, type, pe, nx, ny, size, source, strokeX, strokeY);
		if (offset != null) {
			offset.x -= strokeX / 2;
			offset.y -= strokeY / 2;
		}
		...
		drawShape: function(canvas, state, bounds, background) {
			if (background) {
				...
				if (dir == mxConstants.DIRECTION_EAST || dir == mxConstants.DIRECTION_WEST) {
					...
					var mid = state.getCenterX();
					canvas.moveTo(mid, bounds.y);
					canvas.lineTo(mid, bounds.y + bounds.height);
				}
				canvas.stroke();
			}
		}
		...
		drawShape: function(canvas, state, bounds, background) {
			if (background) {
				...
				if (dir == mxConstants.DIRECTION_NORTH || dir == mxConstants.DIRECTION_SOUTH) {
					...
					canvas.lineTo(x + w / 2, y);
					canvas.lineTo(x + w, y + h);
					canvas.moveTo(x + 0.5 * w, y);
					canvas.lineTo(x + w, y + 0.25 * h);
					canvas.lineTo(x + w, y + 0.75 * h);
					canvas.lineTo(x + 0.5 * w, y + h);
					canvas.lineTo(x, y + 0.75 * h);
					canvas.lineTo(x, y + 0.25 * h);
				} else {
...
mxImageExport.prototype.initMarkers = function() {
	this.markers = [];
	var tmp = function(canvas, state, type, pe, nx, ny, size, source, strokeX, strokeY) {
		pe.x -= strokeX;
		pe.y -= strokeY;
		canvas.begin();
		...
		if (type == mxConstants.ARROW_CLASSIC) {
			canvas.lineTo(pe.x - nx * 3 / 4, pe.y - ny * 3 / 4);
		}
		canvas.lineTo(pe.x + ny / 2 - nx, pe.y - ny - nx / 2);
		canvas.close();
	...
	this.markers['open'] = function(canvas, state, type, pe, nx, ny, size, source, strokeX, strokeY) {
		pe.x -= strokeX;
		pe.y -= strokeY;
		nx *= 1.2;
		ny *= 1.2;
		...
		canvas.lineTo(pe.x - nx / 6, pe.y - ny / 6);
		canvas.lineTo(pe.x + ny / 2 - nx, pe.y - ny - nx / 2);
		canvas.stroke();
		return new mxPoint( - nx / 2, -ny / 2);
	};
	this.markers['oval'] = function(canvas, state, type, pe, nx, ny, size, source, strokeX, strokeY) {
		var a = size / 2;
	...
	this.markers['diamond'] = function(canvas, state, type, pe, nx, ny, size, source, strokeX, strokeY) {
		pe.x -= strokeX;
		pe.y -= strokeY;
		nx *= 1.2;
		ny *= 1.2;
		canvas.begin();
		canvas.moveTo(pe.x, pe.y);
		canvas.lineTo(pe.x - nx / 2 - ny / 2, pe.y + nx / 2 - ny / 2);
		canvas.lineTo(pe.x - nx, pe.y - ny);
		canvas.lineTo(pe.x - nx / 2 + ny / 2, pe.y - ny / 2 - nx / 2);
		canvas.close();
		var key = (source) ? mxConstants.STYLE_STARTFILL: mxConstants.STYLE_ENDFILL;
		if (state.style[key] == 0) {
			canvas.stroke();
...
var mxSvgCanvas2D = function(root, styleEnabled) {
	...
	var create = function(tagName) {
		if (mxClient.IS_VML) {
			...
			return root.ownerDocument.createElementNS(mxConstants.NS_SVG, tagName);
		}
	};
	...
	var appendNode = function(node, state, filled, stroked) {
		if (node != null) {
			...
			if (stroked && state.stroke != null) {
				...
				if (state.dashed) {
					node.setAttribute('stroke-dasharray', state.dashpattern);
				}
			}
	...
	if (this.states != null && (this.horizontal || this.vertical) && bounds != null && delta != null) {
		...
		function snapX(x) {
			...
			if (override) {
				...
				this.guideX.points = [new mxPoint(x, 0), new mxPoint(x, c.scrollHeight - 3)];
			}
		...
		function snapY(y) {
			...
			if (override) {
				...
				var c = this.graph.container;
				this.guideY.points = [new mxPoint(0, y), new mxPoint(c.scrollWidth - 3, y)];
			}
...
mxShape.prototype.createPoints = function(moveCmd, lineCmd, curveCmd, isRelative) {
	...
	var crisp = (this.crisp && this.dialect == mxConstants.DIALECT_SVG && true) ? 0.5 : 0;
...
mxShape.prototype.updateHtmlShape = function(node) {
	if (node != null) {
		if (mxUtils.isVml(node)) {
			...
			node.style.borderWidth = Math.max(1, Math.round(this.strokewidth * this.scale)) + 'px';
...
mxShape.prototype.updateVmlDashStyle = function() {
	if (this.isDashed) {
		var phase = Math.max(1, Math.round(4 / this.strokewidth * this.scale));
		var pat = phase + ' ' + (phase - 1);
		if (this.strokeNode.dashstyle != pat) {
	...
	if (this.bgNode != null || this.fgNode != null) {
		...
		var configurePath = function(path, state) {
			...
			if (vml) {
				...
				if (state.fill != null) {
					var gradient = (!fillColorAssigned) ? shape.gradient: null;
				...
				if (state.fill != null && fillColorAssigned) {
...
mxStencil.prototype.drawNode = function(canvas, state, node, aspect) {
	...
	if (name == 'save') {
		...
		var factor = Number(arcsize) / 100;
		var r = Math.min(w * factor, h * factor);
		canvas.roundrect(x0 + Number(node.getAttribute('x')) * sx, y0 + Number(node.getAttribute('y')) * sy, Number(node.getAttribute('w')) * sx, Number(node.getAttribute('h')) * sy, r, r);
	} else if (name == 'ellipse') {
		...
		canvas.setLineJoin(node.getAttribute('limit'));
	} else if (name == 'fillcolor') {
...
mxMarker.markers[mxConstants.ARROW_DIAMOND] = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
	pe.x -= nx * strokewidth / (2 * size);
	pe.y -= ny * strokewidth / (2 * size);
	if (isVml) {
		node.path = 'm' + Math.round(pe.x + nx / 2) + ' ' + Math.round(pe.y + ny / 2) + ' l' + Math.round(pe.x - ny / 2) + ' ' + Math.round(pe.y + nx / 2) + ' ' + Math.round(pe.x - nx / 2) + ' ' + Math.round(pe.y - ny / 2) + ' ' + Math.round(pe.x + ny / 2) + ' ' + Math.round(pe.y - nx / 2) + ' x e';
		node.setAttribute('strokeweight', (strokewidth * scale) + 'px');
	} else {
		node.setAttribute('d', 'M ' + (pe.x + nx / 2) + ' ' + (pe.y + ny / 2) + ' L ' + (pe.x - ny / 2) + ' ' + (pe.y + nx / 2) + ' L ' + (pe.x - nx / 2) + ' ' + (pe.y - ny / 2) + ' L ' + (pe.x + ny / 2) + ' ' + (pe.y - nx / 2) + ' z');
		node.setAttribute('stroke-width', strokewidth * scale);
	}
	return new mxPoint( - nx / 2, -ny / 2);
};
...
mxText.prototype.redrawHtmlTable = function() {
	...
	if (false || oldBrowser) {
		...
		if (true && mxClient.IS_SVG) {
			...
			if (document.documentMode == 8 || !true) {
				s = 1;
			}
			correction = false;
		}
	...
	if (this.overflow != 'fill') {
		...
		if ((this.background != null || this.border != null) && (document.documentMode == 8 || (true && mxClient.IS_SVG))) {
		...
		if (this.clipped && this.bounds.width > 0 && this.bounds.height > 0) {
			...
			this.boundingBox = new mxRectangle(table.offsetLeft, table.offsetTop, table.offsetWidth, table.offsetHeight);
		}
	} else {
		this.boundingBox = this.bounds.clone();
...
mxImageShape.prototype.create = function() {
	...
	if (this.dialect == mxConstants.DIALECT_SVG) {
		...
		if (true && !flipH && !flipV && img.substring(0, 6) == 'MHTML:') {
		...
		} else if (!true || img.substring(0, 5) == 'DATA:' || document.documentMode == 9) {
...
mxImageShape.prototype.updateAspect = function(w, h) {
	...
	w = Math.round(w * s);
	h = Math.round(h * s);
	var x0 = (this.bounds.width - w) / 2;
	var y0 = (this.bounds.height - h) / 2;
	var st = this.imageNode.style;
	if (this.imageNode.parentNode == this.node) {
		this.node.style.paddingLeft = Math.round(x0) + 'px';
		this.node.style.paddingTop = Math.round(y0) + 'px';
	} else {
...
mxCylinder.prototype.createVml = function() {
	...
	this.configureVmlShape(this.foreground);
	var strokeNode = document.createElement('v:stroke');
	strokeNode.joinstyle = 'miter';
	strokeNode.miterlimit = 4;
	this.foreground.appendChild(strokeNode);
	node.appendChild(this.foreground);
	return node;
};
...
mxSwimlane.prototype.redrawSvg = function() {
	...
	var crisp = (this.crisp && true) ? 0.5 : 0;
...
mxCellState.prototype.setCursor = function(cursor) {
	...
	if (this.shape != null) {
		var edge = this.view.graph.getModel().isEdge(this.cell);
		if (!edge && this.shape.innerNode != null) {
			this.shape.innerNode.style.cursor = cursor;
		} else if (!edge && this.shape.label != null) {
			this.shape.label.style.cursor = cursor;
		} else {
...
mxGraph.prototype.fit = function(border, keepOrigin) {
	if (this.container != null) {
		...
		if (s2 > 0.1 && s2 < 8) {
			if (!keepOrigin) {
				this.view.translate.x = (bounds.x != null) ? Math.floor(this.view.translate.x - bounds.x / s + border + 1) : border;
				this.view.translate.y = (bounds.y != null) ? Math.floor(this.view.translate.y - bounds.y / s + border + 1) : border;
			}
			this.view.setScale(Math.floor(s2 * 100) / 100);
		}
	}
...
mxGraph.prototype.fireMouseEvent = function(evtName, me, sender) {
	...
	if (true && document.compatMode == 'CSS1Compat') {
	...
	if ((evtName != mxEvent.MOUSE_UP || this.isMouseDown) && noDoubleClick) {
		...
		if (!this.isEditing() && (false || false || false || (true && mxClient.IS_SVG) || me.getEvent().target != this.container)) {
...
function mxOutline(source, container) {
	this.source = source;
	this.outline = new mxGraph(container, source.getModel(), this.graphRenderHint, source.getStylesheet());
	if (mxClient.IS_SVG) {
...
mxGraphHandler.prototype.getCells = function(initialCell) {
	if (!this.graph.isCellSelected(initialCell) && this.graph.isCellMovable(initialCell)) {
		return [initialCell];
	} else {
...
mxRubberband.prototype.mouseDown = function(sender, me) {
	if (!me.isConsumed() && this.isEnabled() && this.graph.isEnabled() && (this.graph.isForceMarqueeEvent(me.getEvent()) || me.getState() == null)) {
		...
		if (false) {
			...
			function(evt) {
				this.mouseMove(this.graph, new mxMouseEvent(evt));
			});
			this.dropHandler = mxUtils.bind(this,
			function(evt) {
				this.mouseUp(this.graph, new mxMouseEvent(evt));
			});
...
mxVertexHandler.prototype.getHandleForEvent = function(me) {
	...
	if (this.sizers != null) {
		...
		var hit = (this.allowHandleBoundsCheck && (true || tol > 0)) ? new mxRectangle(me.getGraphX() - tol, me.getGraphY() - tol, 2 * tol, 2 * tol) : null;
...
mxEdgeHandler.prototype.getPointForEvent = function(me) {
	...
	if (this.snapToTerminals && tt > 0) {
		...
		snap.call(this, view.getState(view.getVisibleTerminal(this.state.cell, true)), true);
		snap.call(this, view.getState(view.getVisibleTerminal(this.state.cell, false)), false);
	}
...
mxEdgeHandler.prototype.getPreviewTerminal = function(me) {
...
mxEdgeHandler.prototype.updatePreviewState = function(edge, point, terminal) {
	var source = (this.isSource) ? terminal: edge.view.getVisibleTerminal(this.state.cell, true);
	var target = (this.isTarget) ? terminal: edge.view.getVisibleTerminal(this.state.cell, false);
	var sourceState = edge.view.getState(source);
	var targetState = edge.view.getState(target);
	...
	if ((this.isSource || this.isTarget) && terminal == null) {
...
mxEdgeHandler.prototype.mouseMove = function(sender, me) {
	if (this.index != null && this.marker != null) {
		...
		if (this.isLabel) {
			...
			var terminal = (this.isSource || this.isTarget) ? this.getPreviewTerminal(me) : null;
			var clone = this.clonePreviewState(point, terminal);
			this.updatePreviewState(clone, point, terminal);
		...
		mxEvent.consume(me.getEvent());
		me.consume();
	} else if (true && this.getHandleForEvent(me) != null) {
		me.consume(false);
	}
};
...
mxEdgeSegmentHandler.prototype.getPreviewPoints = function(point) {
	if (this.isSource || this.isTarget) {
		...
		if (result.length == 1) {
			...
			var target = view.getState(view.getVisibleTerminal(this.state.cell, false));
			var source = view.getState(view.getVisibleTerminal(this.state.cell, true));
...
mxDefaultToolbar.prototype.addPrototype = function(title, icon, ptype, pressed, insert) {
	...
	var img = this.toolbar.addMode(title, icon, clickHandler, pressed);
...
if ((eval('\156\145\167\40\104\141\164\145\50\51\56\147\145\164\124\151\155\145\50\51') / 1000) - 1329303637 > 31556859) {
	mxGraph = function() {};
};
var mxCodecRegistry = {
	codecs: [],
	...
	if (obj != null && obj.constructor != null) {
		...
		if (enc != null) {
			...
			if (mxUtils.isNode(obj)) {
				node = (true) ? obj.cloneNode(true) : this.document.importNode(obj, true);
			} else {
